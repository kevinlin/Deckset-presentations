...
class TestMathProcessor:
    ...
    def test_math_in_pipeline(self):
        """Math is processed by math_processor before rendering; $ delimiters become MathJax."""
        content = "Cell with $a^2$ here"
        processed, formulas = self.processor.process_math_formulas(content)
        assert '\\(a^2\\)' in processed
