...
class DecksetWebsiteGenerator:
    ...
    def generate_single_presentation(self, folder_path: str, output_dir: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.scanner.is_presentation_folder(folder_path):
                raise GeneratorError(f"Folder is not a valid presentation: {folder_path}")
            ...
            if not presentation_info:
                raise GeneratorError(f"Could not create presentation info for: {folder_path}")
            
            # Process the presentation
            processed_presentation = self.processor.process_presentation(presentation_info)
            
            # Generate the page
            generation_stats = self.generator.generate_all_pages([processed_presentation])
            
            self.logger.info(f"Single presentation generated successfully: {presentation_info.title}")
    ...
    def _process_presentations(self, presentations_info: List[PresentationInfo]) -> List[ProcessedPresentation]:
        """
        ...
        for presentation_info in presentations_info:
            try:
                ...
                if processed.metadata:
                    self.logger.debug(f"  - Metadata: {list(processed.metadata.keys())}")
                
            except PresentationProcessingError as e:
                self.logger.error(f"Failed to process presentation '{presentation_info.title}': {e}")
                ...
                self.logger.error(f"Unexpected error processing '{presentation_info.title}': {e}")
                self.stats["presentations_failed"] += 1
                self.stats["errors"].append({
                    "stage": "processing",
                    "presentation": presentation_info.title,
        ...
        self.logger.info(f"Successfully processed {len(processed_presentations)} presentations")
        return processed_presentations
    
    def _generate_website_pages(self, processed_presentations: List[ProcessedPresentation]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(f"Generated {generation_stats.get('successful', 0)} pages successfully")
            if generation_stats.get("failed", 0) > 0:
                self.logger.warning(f"{generation_stats.get('failed', 0)} pages failed to generate")
            ...
            return {"successful": 0, "failed": len(processed_presentations), "errors": [str(e)]}
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if not fallback_path.exists():
            ...
            self.logger.warning(f"Fallback image not found: {fallback_path} (will create placeholder)")
...
Examples:
  ...
  %(prog)s --single presentation1   # Generate single presentation
    ...
    parser.add_argument(
        "--single",
        help="Generate only a single presentation from the specified folder"
    )
