...
- [x] Write failing test `tests/test_theme_contract.py::test_theme_files_are_var_only`: parse each `templates/assets/css/themes/*.css`, assert the only selector is `:root` (regex `^\s*([^{}]+)\s*\{` over the file; every capture strips to `:root`).
- [x] Write failing test `test_light_theme_defines_required_vars`: `light.css` defines every variable in the contract list above (assert `--color-canvas:` etc. present).
- [x] Rewrite `light.css` as the full token set using today's effective light values from `slide_styles.css` (canvas `#fff`, ink `#374151`, ink-muted `#4b5563`, ink-subtle `#6b7280`, accent/link `#3b82f6`, hairline `#e5e7eb`, code-bg `#f3f4f6`, table-header-bg `#f9fafb`, body font = current system stack, mono = `'Monaco', 'Menlo', 'Ubuntu Mono', monospace`, h1/h2/h3 sizes = current rem values converted to em).
- [x] Rewrite `dark.css` and `minimal.css` as `:root` token sets reproducing their current palettes (dark: canvas `#1a1a2e`, ink `#e0e0e0`, code-bg `#0d1117`, accent `#4a90d9`, link `#6db3f2`, table-header-bg `#21262d`, hairline `#30363d`; minimal: light palette + `--font-display: Georgia, 'Times New Roman', serif`, muted `#666`, hairline `#ccc`).
- [x] Re-plumb `slide_styles.css`: every color and font-family becomes `var(--<role>, <current literal>)` per the D1 mapping (slide background → canvas, body text → ink, headings → ink + display font/sizes/weights, links → link, blockquote border → blockquote-border, code/pre → code-bg + mono, table header → table-header-bg, borders → hairline, footer/slide-number → ink-subtle). Leave non-themable rgba overlays (image filters, `--overlay-opacity` machinery) untouched.
- [x] Re-plumb `homepage.css` hover shadow to use `--color-hairline` where it hardcodes rgba black at 10%; keep the rest.
- [x] Run `uv run pytest tests/test_theme_contract.py tests/test_regression_site.py -v` — all pass.
- [x] Manual spot check: build site, open one deck with `theme: dark` and one without; visuals match pre-change.
- [x] Commit: `refactor: move base CSS onto theme variable contract`
...
- [x] `uv add pyyaml`.
- [x] Failing tests, then implement `parse_design_file`:
...
- [x] Failing tests, then implement `resolve_references`:
...
- [x] Sanity test `test_all_repo_designs_parse` — loop `design-md/*/DESIGN.md`, `resolve_references(parse_design_file(p))` succeeds for all 13.
- [x] `uv run pytest tests/test_theme_compiler.py -v` green; commit: `feat: theme compiler parses design-md DESIGN.md files`
...
- [x] Failing tests, then implement, in this order:
...
- [x] `test_compile_all_real_designs` — run against repo `design-md/`: 13 CSS files, manifest has 16 entries.
- [x] `uv run pytest tests/test_theme_compiler.py -v` green; commit: `feat: compile design-md tokens to theme CSS and manifest`
...
- [x] Add `theme: str = "light"` and `designs_dir: str = "design-md"` fields to `GeneratorConfig` (`src/models.py:46`).
- [x] Failing test `tests/test_main.py::test_theme_cli_flag` — `--theme dark` sets `config.theme == "dark"`; then add `--theme` to the main.py argparser (default `"light"`) and thread into `GeneratorConfig`.
- [x] Failing tests for registry resolution in `tests/test_enhanced_templates.py`: after `set_theme_registry({"light","dark","minimal","linear-app"}, "dark")` — `resolve_theme("linear-app") == "linear-app"`, `resolve_theme(None) == "dark"`, `resolve_theme("nope") == "dark"` with warning (caplog). Implement; delete the filesystem probe at `enhanced_templates.py:112-114` and the `KNOWN_THEMES` constant.
- [x] Invoke compiler in `WebPageGenerator.generate_all_pages()` (`src/generator.py:308`) before page generation: `ThemeCompiler(Path(self.config.designs_dir)).compile_all(Path(self.config.output_dir)/"assets"/"css"/"themes")`; store manifest on `self.theme_manifest`; call `template_manager.set_theme_registry({t.slug for t in manifest}, self.config.theme)`. Both full build (`main.py:327`) and `--single` (`main.py:198`) route through here — no extra wiring.
- [x] `theme_stylesheets()` (`enhanced_templates.py:118`) emits the theme link as `<link id="theme-css" rel="stylesheet" href="{prefix}assets/css/themes/{theme}.css" data-default-href="{same}">`; custom.css link unchanged.
- [x] Homepage gets the site-default theme: add to `generate_homepage()` context `default_theme=self.template_manager.resolve_theme(self.config.theme)`; in `templates/homepage.html` after the `homepage.css` link add `<link id="theme-css" rel="stylesheet" href="assets/css/themes/{{ default_theme }}.css" data-default-href="assets/css/themes/{{ default_theme }}.css">`.
- [x] `--validate` (`main.py:436` `required_assets`): add `assets/css/themes/dark.css`, `assets/css/themes/minimal.css` (built-in templates; compiled themes are build-time-validated, D4).
- [x] Regression harness additions (same commit): built site has `assets/css/themes/themes.json` and ≥16 `themes/*.css`; homepage links `themes/light.css` with `id="theme-css"`; a fixture deck with `theme: notion` links `themes/notion.css`.
- [x] `uv run pytest` green; `uv run python src/main.py --validate` exits 0; commit: `feat: theme registry, --theme flag, compiled themes in build`
...
- [x] Add to `homepage.css` (loaded by both pages after this task — also add `<link rel="stylesheet" href="{{ asset_path_prefix }}assets/css/homepage.css">` to `presentation.html`):
...
- [x] In both templates, strip the color utilities these classes replace (`bg-white`, `bg-gray-50`, `text-gray-900/600/500`, `hover:text-gray-900`, `border-b`, `text-blue-700 bg-blue-100 hover:bg-blue-200`, card `bg-white`) and add the new class names (`site-header`, `site-nav-link`, `site-footer`, `card-title`, `card-meta`, `card-cta`, `body` styles come free). Layout utilities (`flex`, `px-*`, `max-w-*`, `shadow-*`, `rounded-md` on non-themed bits) stay.
- [x] Harness (same commit): assert homepage header has `class~="site-header"` and no `bg-white` on it; presentation page header likewise.
- [x] `uv run pytest` green; manual: build, homepage renders identically under light; commit: `refactor: theme-variable classes replace header color utilities`
...
- [x] Jest failing tests in `tests/test_theme_switcher.js` (jsdom, mirror existing viewer test setup): populates options from inline manifest (17 incl. "Page default"); selecting `notion` swaps href to `…/themes/notion.css` and persists; stored value applied on load; "Page default" clears storage and restores `data-default-href`; missing manifest script is a no-op.
- [x] Implement `theme-switcher.js`; add `<script src="…assets/js/theme-switcher.js"></script>` before `</body>` in both templates (homepage: plain path, presentation: `{{ asset_path_prefix }}` prefix).
- [x] Add manifest injection in `generator.py` (both contexts) and the `<select>` + manifest `<script>` tags to both templates. Style `#theme-select` in `homepage.css` with var-driven colors (`background: var(--color-surface-1); color: var(--color-ink); border: 1px solid var(--color-hairline); border-radius: var(--radius-sm)`).
- [x] `--validate`: add `assets/js/theme-switcher.js` to `required_assets` (`main.py:436`).
- [x] Harness (same commit): both page types contain `#theme-select`, `#theme-manifest`, and the switcher script tag.
- [x] `npm test` and `uv run pytest` green; commit: `feat: visitor theme switcher with localStorage persistence`
...
- [x] README theme section: list the 16 themes, `--theme` flag, deck `theme:` directive, visitor switcher, how to add a design (drop a getdesign.md-format folder into `design-md/`), non-consumed sections (`spacing`, `components`), font-stack substitution note.
- [x] Manual pass (D6): build the repo site; on homepage flip through ≥4 themes including `linear-app` (near-black) and `notion` (light) — check card/text contrast; open one deck, switch themes, navigate slides, check code block + table readability; print-preview one deck (switcher hidden by existing `no-print` handling — add class if missed); reload page and confirm persisted theme applies.
- [x] Coverage: `uv run pytest --cov=src --cov-report=term` — `theme_compiler.py` ≥ 85%.
- [x] Full verify: `uv run pytest && npm test && uv run python src/main.py --validate`.
- [x] Commit: `docs: theme system usage and spec status`
